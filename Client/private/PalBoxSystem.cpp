...
void Client::CPalBoxSystem::MovePalSlotUI()
{
	...
	std::wstring wstrCurLeftClickedTextureName	 = m_wpCurLeftClickedSlotComponent.lock()->GetTextureName();
	std::wstring wstrCurLeftClickedSlotComponentName = m_wpCurLeftClickedSlotComponent.lock()->GetComponentName();
	std::wstring wstrCurLeftClickedComponentName = ExceptLastChar(wstrCurLeftClickedSlotComponentName, 2);
	std::wstring wstrPreLeftClickedComponentName = ExceptLastChar(m_wstrPreClickedComponentName, 2);
...
#pragma endregion
	...
	else 
	{
		...
		if ( wstrCurLeftClickedTextureName == L"T_pal_icon_none" ||
			(wpIconComponent.lock()->IsActive() == false && 
			 wstrCurLeftClickedComponentName == L"left_none_hover_"))
		{
			/* LEFT PANEL */
			if (wstrPreLeftClickedComponentName == L"left_none_hover_")
			{
				MovePalFromLeftPanel();
			}
			/* BOTTOM PANEL */
			else if (wstrPreLeftClickedComponentName == L"bottom_pal_slot_")
			{
				MovePalFromBottomPanel();
			}
			/* MID PANEL */
			else if (wstrPreLeftClickedComponentName == L"mid_pal_slot_")
			{
				MovePalFromMidPanel();
			}
		}
...
void Client::CPalBoxSystem::MovePalFromLeftPanel()
{
}

void Client::CPalBoxSystem::MovePalFromMidPanel()
{
}

void Client::CPalBoxSystem::MovePalFromBottomPanel()
{
}
