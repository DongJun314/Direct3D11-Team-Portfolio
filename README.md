<h1 align="center">🕹️ Direct3D 11 Team Portfolio – Map Tool & System Demo</h1>
<p align="center">
  This project was developed to gain <strong>hands-on experience</strong> with <strong>DirectX 11</strong>
  and to deepen understanding of <strong>map creation and rendering</strong>.<br>
  We chose an <strong>open-world survival genre</strong> to challenge ourselves with complex system architectures
  and large-scale structures.
</p>

<p align="center">
  <a href="./README.md">English</a> |
  <a href="./README_jp.md">日本語</a>
</p>


## 🌟 Highlights (Map Tool)
<table>
  <tr>
    <td style="text-align:center">
      <img src="https://github.com/user-attachments/assets/05c23aa8-aff3-450b-98fb-9cba7951aa53" width="400" height="300">
    </td>
    <td style="text-align:center">
      <img src="https://github.com/user-attachments/assets/03ac4e14-8996-4344-bceb-419fb5801bf1" width="400" height="300">
    </td>
  </tr>
</table>

<br>

## 🔗 Link
- [Portfolio Video](https://youtu.be/NmdzlMohV5g)

<br>

## 📜 Project Overview
- **Duration**: `2024.03.18 ~ 2024.05.24`
- **Team Size**: `5 members`
- **Key Contributions**: `Map Tool`, `Inventory System`, `Inventory UI`

<br>

## ⚙️ Development Environment
- **Languages**: `C++`, `HLSL`
- **Tools**: `Visual Studio 2022`
- **Version Control**: `Git`, `GitHub`
- **Rendering API**: `Direct3D 11`

<br>

## ⚒️ Implementations
- **Map Tool**: Implemented various widgets and functions for efficient object placement and editing.
- **Pixel Picking**: Implemented a pixel-picking system that retrieves world coordinates at a constant cost regardless of object count.
- **Inventory System**: Designed as an independent system class, managing items and UI separately from player objects.

<br>

## ⚠️ Notes
- This project was developed for **educational purposes** and is **non-public release**.
- **All copyright-sensitive materials** within the videos and documentation
have been **excluded or replaced** to address any potential copyright concerns regarding assets or UI elements.
